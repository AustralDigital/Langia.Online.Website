...
const content = pagesContent.es.kidsTeens.kidsTeensPage;
const imagePath = "/images/programs/kids-teens.webp";
const hasProgramImage = existsSync(path.join(process.cwd(), "public", imagePath));

export const metadata: Metadata = {
  title: "Langia 4 Kids n Teens | Live Language Learning for Kids and Teens",
  description:
    "Live small-group language learning for kids and teens ages 8 to 17, with parent reports, playful practice, and structured progress.",
};

function getContent(): KidsTeensPageContent {
  if (!content) {
  ...
  return (
    <div className="relative min-h-[390px] overflow-hidden rounded-[2rem] border border-[#D8E6F4] bg-[#F3F7FB] shadow-[0_24px_80px_rgba(11,31,58,0.08)] lg:min-h-[500px]">
      {hasProgramImage ? (
        ...
        <div className="absolute inset-0 overflow-hidden bg-[linear-gradient(135deg,#FFFFFF_0%,#F3F7FB_50%,#EAF6FF_100%)]">
          ...
          <div className="absolute right-8 top-20 h-32 w-[54%] rounded-[1.75rem] border border-white/70 bg-white/72 shadow-[0_18px_48px_rgba(11,31,58,0.09)] backdrop-blur-md" />
          <div className="absolute bottom-9 left-8 h-44 w-[64%] rounded-[1.75rem] border border-white/70 bg-white/82 shadow-[0_22px_58px_rgba(11,31,58,0.1)] backdrop-blur-md" />
          <div className="absolute bottom-28 left-14 h-3 w-32 rounded-full bg-[#048EFF]/35" />
          <div className="absolute bottom-20 left-14 h-3 w-48 max-w-[58%] rounded-full bg-[#D8E6F4]" />
          <div className="absolute bottom-12 left-14 h-3 w-36 rounded-full bg-[#D8E6F4]" />
      ...
      <div className="absolute inset-x-5 bottom-5 rounded-[1.5rem] border border-white/60 bg-white/86 p-5 shadow-[0_18px_46px_rgba(11,31,58,0.1)] backdrop-blur-xl">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#048EFF]">Parent-visible progress</p>
        <p className="mt-2 max-w-md font-heading text-xl font-semibold leading-snug text-[#0B1F3A]">
          Small groups, monthly reports, and age-aware guided practice.
        </p>
      </div>
    </div>
  );
...
function SectionHeader({ eyebrow, title, body, light = false }: { eyebrow: string; title: string; body?: string; light?: boolean }) {
  return (
    <div className="max-w-3xl">
      ...
      <h2 className={`mt-4 font-heading text-3xl font-semibold leading-tight sm:text-4xl ${light ? "text-white" : "text-[#0B1F3A]"}`}>
        {title}
      </h2>
...
export default function LangiaForKidsAndTeensPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-[#0B1F3A]">
        ...
        <div className="mx-auto grid max-w-[1180px] gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Button href="#pricing" variant="secondary">{page.hero.secondaryCta}</Button>
            </div>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {page.formats.cards.map((card) => (
              <article key={card.title} className="rounded-[1.5rem] border border-[#D8E6F4] bg-white p-6 shadow-[0_16px_44px_rgba(11,31,58,0.055)]">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#048EFF]">Small group</p>
                <h3 className="mt-3 font-heading text-3xl font-semibold text-[#0B1F3A]">{card.title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#42526A]">{card.body}</p>
                <p className="mt-5 rounded-2xl bg-[#F3F7FB] p-4 text-sm leading-7 text-[#0B1F3A]">{card.bestFor}</p>
              </article>
            ))}
          </div>
        </div>
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {page.includes.features.map((feature) => (
              <article key={feature.title} className="rounded-[1.35rem] border border-[#E4EDF7] bg-white p-5 shadow-[0_14px_38px_rgba(11,31,58,0.05)]">
                <span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#EAF6FF] text-[#048EFF]"><CheckIcon /></span>
                <h3 className="mt-5 font-heading text-lg font-semibold leading-tight text-[#0B1F3A]">{feature.title}</h3>
      ...
      <section className="bg-[#F3F7FB] px-4 py-16 sm:px-6 lg:px-10 lg:py-20">
        <div className="mx-auto max-w-[1180px]">
          <SectionHeader {...page.classFeel} />
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {page.classFeel.cards.map((card) => (
              <article key={card.title} className="rounded-[1.5rem] border border-[#D8E6F4] bg-white p-6 shadow-[0_16px_44px_rgba(11,31,58,0.055)]">
                <h3 className="font-heading text-xl font-semibold text-[#0B1F3A]">{card.title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#42526A]">{card.body}</p>
              </article>
      ...
      <section className="bg-white px-4 py-16 sm:px-6 lg:px-10 lg:py-20">
        <div className="mx-auto max-w-[1180px]">
          <SectionHeader {...page.parentSafety} />
          <div className="mt-10 grid gap-5 md:grid-cols-4">
            {page.parentSafety.cards.map((card) => (
              <article key={card.title} className="rounded-[1.5rem] border border-[#E4EDF7] bg-white p-6 shadow-[0_16px_44px_rgba(11,31,58,0.055)]">
                <h3 className="font-heading text-xl font-semibold text-[#0B1F3A]">{card.title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#42526A]">{card.body}</p>
              </article>
          ...
          <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {page.levels.items.map((level, index) => (
              <div key={level} className="rounded-[1.25rem] border border-[#D8E6F4] bg-white p-5 shadow-[0_12px_34px_rgba(11,31,58,0.045)]">
                <span className="text-xs font-semibold uppercase tracking-[0.16em] text-[#048EFF]">Level {index + 1}</span>
                <h3 className="mt-3 font-heading text-lg font-semibold leading-tight text-[#0B1F3A]">{level}</h3>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-3">
            {page.schedule.bullets.map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-2xl border border-[#E4EDF7] bg-white p-4 shadow-[0_12px_34px_rgba(11,31,58,0.045)]">
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#EAF6FF] text-[#048EFF]"><CheckIcon /></span>
      ...
      <section id="pricing" className="bg-[#F3F7FB] px-4 py-16 sm:px-6 lg:px-10 lg:py-20">
        <div className="mx-auto max-w-[1180px]">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            <div className="rounded-[1.5rem] border border-[#D8E6F4] bg-white p-6"><h3 className="font-heading text-xl font-semibold text-[#0B1F3A]">{page.pricing.studentCreditTitle}</h3><p className="mt-3 text-sm leading-7 text-[#42526A]">{page.pricing.studentCreditBody}</p></div>
            <div className="rounded-[1.5rem] border border-[#D8E6F4] bg-white p-6"><h3 className="font-heading text-xl font-semibold text-[#0B1F3A]">{page.pricing.payFullTitle}</h3><p className="mt-3 text-sm leading-7 text-[#42526A]">{page.pricing.payFullBody}</p></div>
          </div>
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            {page.pricing.cards.map((card) => (
              <article key={card.title} className="relative rounded-[1.75rem] border border-[#D8E6F4] bg-white p-6 shadow-[0_18px_50px_rgba(11,31,58,0.06)]">
                {card.badge ? <span className="absolute right-5 top-5 rounded-full bg-[#F3B737]/18 px-3 py-1 text-xs font-semibold text-[#8A6200]">{card.badge === "personalized" ? page.pricing.labels.personalized : page.pricing.labels.value}</span> : null}
                <h3 className="font-heading text-3xl font-semibold text-[#0B1F3A]">{card.title}</h3>
                <dl className="mt-7 grid gap-4 text-sm">
                  {[[page.pricing.labels.base, card.studentCreditPerLesson], [page.pricing.labels.total, card.studentCreditTotal], [page.pricing.labels.installments, card.installments], [page.pricing.labels.payFull, card.payFull]].map(([label, value]) => (
                    <div key={label} className="rounded-2xl bg-[#F3F7FB] p-4"><dt className="text-xs font-semibold uppercase tracking-[0.12em] text-[#42526A]">{label}</dt><dd className="mt-2 font-heading text-lg font-semibold text-[#0B1F3A]">{value}</dd></div>
                  ))}
                </dl>
                <div className="mt-7"><Button href="/contact">{page.pricing.cta}</Button></div>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white px-4 py-16 sm:px-6 lg:px-10">
        <div className="mx-auto grid max-w-[1180px] gap-8 rounded-[2rem] bg-[#0B1F3A] p-8 text-white shadow-[0_28px_90px_rgba(11,31,58,0.2)] md:grid-cols-[1fr_auto] md:items-center sm:p-10 lg:p-12">
          <SectionHeader {...page.schools} light />
          <Button href="/contact" variant="dark">{page.schools.cta}</Button>
        </div>
      </section>
      ...
      <section className="bg-[#F3F7FB] px-4 py-16 sm:px-6 lg:px-10">
        <div className="mx-auto grid max-w-[1180px] gap-8 rounded-[2rem] bg-[#0B1F3A] p-8 text-white shadow-[0_28px_90px_rgba(11,31,58,0.22)] md:grid-cols-[1fr_auto] md:items-center sm:p-10 lg:p-12">
          ...
          <div className="flex flex-col gap-3 sm:flex-row"><Button href="/contact">{page.finalCta.primaryCta}</Button><Button href="/programs" variant="dark">{page.finalCta.secondaryCta}</Button></div>
        </div>
      </section>
